use rs_aoc::days::dayXX::DayXX;
use rs_aoc::days::Solution;
use rs_aoc::{read_input, read_sample_input};

#[test]
fn test_dayXX_part1_sample() {
    let input = read_sample_input(XX);
    let result = DayXX.part1(&input);
    assert_eq!(result, "expected"); // TODO: replace with expected sample answer
}

#[test]
fn test_dayXX_part2_sample() {
    let input = read_sample_input(XX);
    let result = DayXX.part2(&input);
    assert_eq!(result, "expected"); // TODO: replace with expected sample answer
}

#[test]
#[ignore] // remove once you have the real answer
fn test_dayXX_part1_real() {
    let input = read_input(XX);
    let result = DayXX.part1(&input);
